/**
 * 🎮 Gamescan API — vSecure + Cost Tracker (Stable Render Edition)
 * ------------------------------------------------------------
 * ✅ Secure (Auth, Rate Limit, CORS, Helmet)
 * ✅ Cost-efficient model logic (Gamer = Free)
 * ✅ Logs estimated OpenAI cost per analysis
 * ✅ Tracks daily + monthly totals (stable under Render restarts)
 * ------------------------------------------------------------
 */
...
function resetDaily() {
  const now = Date.now();
  if (now - lastDayReset < 60_000) return;
  lastDayReset = now;
  console.log(`📊 Daily total reset → $${dailyCost.toFixed(4)} | Monthly: $${monthlyCost.toFixed(4)}`);
  dailyCost = 0;
}
...
function scheduleDailyReset() {
  ...
  console.log(`🕓 Next daily reset scheduled for: ${next.toUTCString()}`);
  setTimeout(() => {
    resetDaily();
...
function scheduleMonthlyReset() {
  if (monthlyTimerStarted) return;
  monthlyTimerStarted = true;
  const now = new Date();
  const next = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() + 1, 1, 0, 0, 0));
  const delay = next - now;
  console.log(`🕓 Next monthly reset scheduled for: ${next.toUTCString()}`);
  setTimeout(() => {
    resetMonthly();
...
async function loadCache() {
  try {
    ...
    console.error("⚠️ Cache load failed:", e.message);
  }
}
await loadCache();
async function saveCache() {
  try {
    await atomicWrite(CACHE_FILE, JSON.stringify(Object.fromEntries(analysisCache), null, 2));
  } catch (e) {
    console.error("⚠️ Cache save failed:", e.message);
  }
}
setInterval(async () => {
  ...
  analysisCache.forEach((v, k) => {
    if (v.timestamp && now - v.timestamp > ttl) {
  ...
  if (removed) {
    await saveCache();
    console.log(`🧹 Cache cleanup: ${removed}`);
  }
}, 3600000);
...
async function processNext() {
  ...
  job
    .jobFn()
    .then((r) => {
      if (!job.res.headersSent) job.res.json({ queued: false, ...r });
      job.resolve(r);
    })
    .catch((e) => {
      console.error("❌ Job error:", e.message);
      if (!job.res.headersSent)
        job.res.status(500).json({ success: false, error: "Internal" });
      job.reject(e);
    })
    .finally(() => {
      running--;
...
function execWithTimeout(cmd, timeout) {
  return new Promise((res, rej) => {
    exec(cmd, { timeout }, (err, out, errout) => {
...
async function handleAnalysis(req) {
  ...
  function estimateCost(u, m) {
    const t = u?.total_tokens || 0;
    return (t / 1000) * (MODEL_RATES[m] || 0.005);
  }

  let model = "gpt-4o-mini";
  if (userTier === "pro") {
    if (detailLevel === "high") model = "gpt-4o";
  } else if (userTier === "master") {
    model = detailLevel === "high" ? "gpt-4.1" : "gpt-4o";
  }
  console.log(`🎯 Using model: ${model} (tier=${userTier}, detail=${detailLevel})`);

  const weightedFocus = focusList.map((f, i) =>
    `- ${f} (${[40, 30, 20, 15, 10][i] || 10}% importance)`
  ).join("\n");
...
// ---------- START ----------
app.listen(PORT, () =>
  console.log(`✅ Gamescan API live on port ${PORT} (${DEV_MODE ? "DEV" : "PROD"})`)
);
